...
void run_comparison(cublasHandle_t handle, int M, int N, int K) {
    ...
    b2.print();
    bc.print();
    printf("  V2/V1 speedup: %.2fx | V2 vs cuBLAS: %.1f%%\n\n",
           b1.time_ms / b2.time_ms,
           b2.tflops / bc.tflops * 100.0f);
...
int main(int argc, char** argv) {
    ...
    // ---- 性能对比 ----
    if (do_benchmark) {
        printf("\n--- Performance Comparison: V1 vs V2 vs cuBLAS ---\n");
