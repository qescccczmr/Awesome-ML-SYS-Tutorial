...
void run_comparison(cublasHandle_t handle, int M, int N, int K) {
    ...
    GEMMBenchmark b5("V5 Production", M, N, K);
    b5.run([&]{ gemm_v5(d_C, d_A, d_B, M, N, K); });
    ...
    b5.print();
    bc.print();
    printf("  V5/V1: %.2fx | V5/V4: %.2fx | V5 vs cuBLAS: %.1f%% | V5 peak%%: %.1f%%\n\n",
           b1.time_ms / b5.time_ms,
           b4.time_ms / b5.time_ms,
           b5.tflops / bc.tflops * 100.0f,
           b5.peak_pct());
...
int main(int argc, char** argv) {
    ...
    all_passed &= run_correctness_test(handle, 512, 512, 512, "Square 512");
    all_passed &= run_correctness_test(handle, 1024,1024,1024,"Square 1024");
    all_passed &= run_correctness_test(handle, 128, 256, 64,  "Rectangular");
    all_passed &= run_correctness_test(handle, 256, 128, 256, "Rectangular 2");
    ...
    if (do_benchmark) {
        ...
        run_comparison(handle, 4096, 2048, 1024);
    }
