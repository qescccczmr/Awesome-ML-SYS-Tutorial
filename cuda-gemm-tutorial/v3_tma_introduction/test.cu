...
void run_comparison(cublasHandle_t handle, int M, int N, int K) {
    ...
    b3.print();
    bc.print();
    printf("  V3/V1 speedup: %.2fx | V3/V2 speedup: %.2fx | V3 vs cuBLAS: %.1f%%\n\n",
           b1.time_ms / b3.time_ms,
           b2.time_ms / b3.time_ms,
           b3.tflops / bc.tflops * 100.0f);
...
int main(int argc, char** argv) {
    ...
    // ---- 性能对比 ----
    if (do_benchmark) {
        printf("\n--- Performance Comparison: V1 vs V2 vs V3 vs cuBLAS ---\n");
